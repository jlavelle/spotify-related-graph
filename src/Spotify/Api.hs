...
getArtist :: MonadHttp m => Token -> SpotifyId -> m (Req.JsonResponse Artist)
getArtist t id = Req.req Req.GET url Req.NoReqBody Req.jsonResponse opts
  where
    url  = rootUrl /: "artists" /: unwrap id
    opts = tokenHeader t

getRelatedArtists :: MonadHttp m => Token -> SpotifyId -> m (Req.JsonResponse ArtistsResponse)
getRelatedArtists t id = Req.req Req.GET url Req.NoReqBody Req.jsonResponse opts
  where
